"""Inspect an external PPTX and export reusable assets plus fusion hints.

This utility is adapted from the lightweight template-import idea used by
`ppt-master`, then reshaped for this repository's template fusion workflow.
It does not attempt full layout conversion. Instead it:

- exports embedded media assets
- summarizes slide size, theme colors, and fonts
- classifies slides into rough page-type candidates
- emits fusion hints for `v2_theme` and `reference_style` extraction
"""
...
THANKS_KEYWORDS = ("thank", "thanks", "q&a", "qa", "contact", "谢谢", "感谢")
...
def parse_theme(root: ET.Element | None) -> dict[str, Any]:
    ...
    if font_scheme is not None:
        ...
        if minor is not None:
            ...
            east_asia = minor.find("a:ea", NS)
            if east_asia is not None and east_asia.attrib.get("typeface"):
                fonts["minorEastAsia"] = east_asia.attrib["typeface"]

    return {"colors": colors, "fonts": fonts}


def choose_common_assets(asset_usage: Counter[str]) -> list[str]:
    return sorted(asset for asset, count in asset_usage.items() if count > 1)


def _reference_style_reason(slide: SlideRecord) -> str:
    if slide.page_type == "toc_candidate":
        return "Stable agenda structure; good candidate for directory or section reference slides."
    if slide.background_asset and slide.image_assets:
        return "Contains both background and image assets; good reference-style extraction candidate."
    if slide.shape_count >= 8:
        return "Has a richer visual structure and is likely reusable as a page pattern."
    return "Layout looks reusable enough to keep as a reference-style candidate."


def build_fusion_hints(slides: list[SlideRecord], theme: dict[str, Any]) -> dict[str, Any]:
    reference_candidates = [
        {
            "slideIndex": slide.index,
            "pageType": slide.page_type,
            "reason": _reference_style_reason(slide),
        }
        for slide in slides
        if slide.page_type in {"toc_candidate", "chapter_candidate", "content_candidate"}
        and (slide.shape_count >= 6 or slide.image_assets or slide.background_asset)
    ]
    reference_ids = {item["slideIndex"] for item in reference_candidates}
    case_library_candidates = [
        slide.index
        for slide in slides
        if slide.index not in reference_ids and slide.page_type == "content_candidate"
    ]

    theme_ready = bool(theme.get("colors") or theme.get("fonts"))
    recommended_actions: list[dict[str, str]] = []
    if theme_ready:
        recommended_actions.append(
            {
                "action": "v2_theme",
                "reason": "Theme colors and fonts were detected; this source can seed a V2 global theme.",
            }
        )
    if reference_candidates:
        recommended_actions.append(
            {
                "action": "reference_style",
                "reason": f"Detected {len(reference_candidates)} slides with reusable composition for reference-style extraction.",
            }
        )
    if case_library_candidates:
        recommended_actions.append(
            {
                "action": "case_library",
                "reason": f"The remaining {len(case_library_candidates)} content slides are better kept as case-library references.",
            }
        )
        ...
        recommended_actions.append(
            {
                ...
                "reason": "Automatic rules did not find stable reusable units; manual selection is recommended.",
            }
        )

    return {
        "themeReady": theme_ready,
        "referenceStyleCandidates": reference_candidates,
        "caseLibraryCandidates": case_library_candidates,
...
def write_analysis(
    ...
    fusion_hints: dict[str, Any],
) -> None:
    lines = [
        f"# External Template Import - {pptx_name}",
        "",
        "## Summary",
    ...
    for action in fusion_hints["recommendedActions"]:
...
def build_import_manifest(pptx_path: Path, output_dir: Path) -> dict[str, Any]:
    with zipfile.ZipFile(pptx_path, "r") as zf:
        ...
        for info in zf.infolist():
            ...
            while destination.exists():
                ...
                counter += 1
            with zf.open(info.filename) as src, destination.open("wb") as dst:
        ...
        for index, slide_path in enumerate(slide_parts, 1):
            ...
            resolved_images = [copied_assets.get(target, PurePosixPath(target).name) for target in image_targets]
        ...
        fusion_hints = build_fusion_hints(slide_records, theme_summary)
        manifest = {
            "source": {
                "pptx": str(pptx_path),
                "name": pptx_path.name,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export reusable assets and fusion hints from an external PPTX template."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Error: failed to import external template: {exc}")
        return 1
    ...
    print(f"Imported external PPTX: {pptx_path.name}")
    print(f"Output directory: {output_dir}")
    print(f"Manifest: {manifest_path.name}")
    print(f"Assets exported: {len(manifest['assets']['allAssets'])}")
    print(f"Reference-style candidates: {len(manifest['fusionHints']['referenceStyleCandidates'])}")
