...
from tools.scenario_generators.build_eu_supply_chain_compliance_slide import build_slide, self_check_layout


class EuSupplyChainComplianceSlideTests(unittest.TestCase):
    def test_build_slide_outputs_single_editable_slide(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            output = Path(tmpdir) / "eu_supply_chain_compliance.pptx"
    ...
    def test_score_json_contains_expected_terms(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            output = Path(tmpdir) / "eu_supply_chain_compliance.pptx"
